...
class Training
{
    ...
    /** Function for inserting Training Request by Users into Database **/
    ...
    /** Showing all the list of Trainings **/
    ...
    public function store($data = [])
    {
        ...
        $stmt->bindParam(4, $data['facilitator']);
        $stmt->bindParam(5, $data['division']);
        if ($stmt->execute()) {
            return $stmt;
        }
    }
    ...
    /** Check the Training Requests **/
    public function checkTrainingRequest($id, $training_title)
    {
        $query = "SELECT * FROM training_request WHERE user_id = ? AND training_title = ? AND (is_approve = '0' OR is_done = '1')";
        $stmt = $this->connect->prepare($query);
        $stmt->bindParam(1, $id);
    ...
    /** Selecting User with their User ID **/
